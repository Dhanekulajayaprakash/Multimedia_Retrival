...
### Option A: Quick Run on Windows (No Docker/Redis Required)
Since setting up Docker/Redis on Windows can be complicated, we have added **Celery Eager Mode Fallback** and automated startup scripts:

1. **One-Time Setup**: Double-click [setup.bat](file:///c:/users/hello/OneDrive/Desktop/retrival/MMDB/setup.bat). This will automatically create a virtual environment `.venv`, install all the dependencies from `requirements.txt`, and download the required spaCy model.
2. **Start the Application**: Double-click [run_app.bat](file:///c:/users/hello/OneDrive/Desktop/retrival/MMDB/run_app.bat). This will spin up:
   - The FastAPI backend (Port 8000)
   - The Streamlit frontend (Port 8501)
3. **Enjoy!** Since Redis is not running, the FastAPI server will automatically process uploaded files synchronously (in eager mode), extracting embeddings/OCR/transcripts inline and writing them straight to LanceDB. You don't need to run any extra workers or Docker containers!

---

### Option B: Running the Full Asynchronous System (Docker + Celery)
If you have Docker Desktop and want to run the full async worker architecture:

1. **Start Services**: Run Redis and LocalStack:
   ```bash
   docker-compose up -d
   ```
