...
def get_clap_model_and_processor():
    global _clap_processor, _clap_model, _device
    if _clap_model is None:
        print("Loading CLAP model...")
        import torch
        _device = "cuda" if (os.environ.get("USE_CUDA") or torch.cuda.is_available()) else "cpu"
        _clap_processor = AutoProcessor.from_pretrained("laion/clap-htsat-unfused")
        _clap_model = ClapModel.from_pretrained("laion/clap-htsat-unfused").to(_device)
...
def embed_audio_chunks(audio_path, chunk_length=10, overlap=3):
    clap_model, clap_processor, DEVICE = get_clap_model_and_processor()
