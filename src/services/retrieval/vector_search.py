...
def _clean_results(df):
    """Join with files table, drop non-serializable columns, and convert all values for JSON output."""
    if not df.empty:
        try:
            ...
            if not files_df.empty:
                ...
                if 'id_file' in df.columns:
                    df = df.drop(columns=['id_file'])
        except Exception as e:
            print(f"Error joining with files table: {e}")
